...
// Add this new method to your live.controller.js
...
const getBroadcastStatus = async (req, res) => {
  try {
    ...
    // Also get stream health information
    ...
    try {
      ...
      if (streamResponse.data.items && streamResponse.data.items.length > 0) {
        streamHealth = streamResponse.data.items[0].status;
      }
    } catch (streamError) {
      console.log('Could not fetch stream health:', streamError.message);
    }
    ...
    res.json({
      ...
      canTransitionToLive: status === 'ready' || status === 'testing',
      isLive: status === 'live'
    });
...
// Updated checkAndGoLive with better error handling and logging
  ...
  try {
    ...
    if (currentStatus === 'ready' || currentStatus === 'testing') {
      try {
        ...
        // Wait a moment and verify the transition worked
        ...
        const newStatus = verifyResponse.data.items[0]?.status?.lifeCycleStatus;
        ...
        let canRetry = true;
        let errorMessage = transitionError.message;

        if (transitionError.message.includes('redundantTransition')) {
        ...
        } else if (transitionError.message.includes('streamNotIngesting')) {
          errorMessage = 'Waiting for RTMP stream to be detected by YouTube';
        } else if (transitionError.message.includes('insufficientPermissions')) {
          errorMessage = 'Insufficient permissions to transition broadcast';
          canRetry = false;
        }
    ...
    // For other statuses, return current status with appropriate message
    let message = `Broadcast is in ${currentStatus} status.`;
    let canRetry = true;

    switch (currentStatus) {
      case 'created':
        message += ' Stream has been created but not yet started.';
        break;
      case 'complete':
      ...
      case 'revoked':
        ...
        break;
      default:
        message += ' Waiting for RTMP stream to start.';
    }
    res.json({
      success: false,
      status: currentStatus,
...
module.exports = {
  ...
  transitionBroadcast,
   getBroadcastStatus,
   checkAndGoLiveEnhanced // Export the enhanced version
};